<x-filament-panels::page>
    ...
    <div class="mt-6">
        ...
        @elseif ($activeTab === 'documents')
            @php
                $documentGroups = [
                    'CNIC Front & Back' => ['cnic', 'cnic_front', 'cnic_back'],
                    'Educational Degree or Transcript' => ['degree', 'transcript'],
                    'CV & Experience Letter' => ['cv', 'experience'],
                ];
            @endphp
            <div class="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm space-y-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h3 class="font-bold text-sm text-navy-900 dark:text-gold-400 uppercase tracking-wider">Staff Documents</h3>
                        <p class="text-xs text-gray-500 mt-1">Missing files do not block onboarding and can be uploaded whenever the teacher provides them.</p>
                    </div>
                    <a href="{{ \App\Filament\Resources\StaffResource::getUrl('edit', ['record' => $record]) }}" class="inline-flex items-center justify-center px-4 py-2 text-xs font-bold bg-navy-900 text-white rounded-lg hover:bg-navy-800">
                        Upload / Replace Documents
                    </a>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    @foreach ($documentGroups as $groupLabel => $types)
                        @php($groupDocuments = $record->documents->whereIn('document_type', $types))
                        <div class="rounded-xl border p-4 {{ $groupDocuments->isNotEmpty() ? 'border-emerald-200 bg-emerald-50/50 dark:border-emerald-900 dark:bg-emerald-950/20' : 'border-amber-200 bg-amber-50/60 dark:border-amber-900 dark:bg-amber-950/20' }}">
                            <div class="flex items-start justify-between gap-3">
                                <h4 class="text-sm font-bold text-gray-900 dark:text-white">{{ $loop->iteration }}. {{ $groupLabel }}</h4>
                                <span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase {{ $groupDocuments->isNotEmpty() ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
                                    {{ $groupDocuments->isNotEmpty() ? 'Uploaded' : 'Missing document' }}
                                </span>
                            </div>
                            <div class="mt-3 space-y-2">
                                @forelse ($groupDocuments as $document)
                                    <a href="{{ \Illuminate\Support\Facades\Storage::disk($document->disk ?: 'public')->url($document->path) }}" target="_blank" class="block text-xs font-semibold text-navy-800 dark:text-gold-300 hover:underline truncate">
                                        {{ $document->stored_filename ?: $document->title }}
                                    </a>
                                @empty
                                    <p class="text-xs text-amber-700 dark:text-amber-300">The admin can upload this later from Edit Staff.</p>
                                @endforelse
                            </div>
                        </div>
                    @endforeach
